...
![Federated Learning](https://img.shields.io/badge/Federated%20Learning-FedAvg%20%7C%20FedBN-green.svg)
...
Financial institutions face a critical dilemma: the need to share fraud patterns to improve security, and the legal obligation to protect sensitive customer data. This project implements a **Federated Learning (FL)** framework that enables multiple "clients" (financial institutions) to collaboratively train a **Deep Neural Network (DNN)** for fraud detection without ever exchanging raw transaction data.

We compare three distinct approaches—**Local Training (No FL)**, **FedAvg**, and **FedBN**—across two benchmark datasets to evaluate performance under heterogeneous data distributions.

**Preprocessing**
-categorical encoding (one-hot/Label)
\_Feature Normalization
-Non-IID splitting into **12 clients**
...
- **Network Layers:**
  ...
  - Input Layer (Feature Scaled/Preprocessed features)
  - Dense Layer (64 neurons, ReLU + Batch Normalization)
  ...
  **Mathematical Formulation of FedAvg:**
  ...
  _(where $w^{(t+1)}$ is the updated global weights, $w_k^{(t)}$ are the local weights of client $k$ at round $t$, $n_k$ is the client sample size, and $n$ is the total sample size across all clients)._
